...
def encode_arithmetic(model, enc, tokenizer, message, prompt=None, embed=None, tokens=None, finish_sent=False, device=None, temp=1.0, precision=16, topk=50000):
    model.eval()
    ...
    with torch.no_grad():
        ...
        while i < len(message) or (finish_sent and not sent_finish):
            ...
            next_token = indices[selection].view(1).unsqueeze(0)
    ...
    bits_per_word = i/total_num_for_stats
    output_list = list(tokens.squeeze().cpu().numpy())

    return output_list, avg_NLL, avg_KL, bits_per_word, avg_Hq, message[:i], i

def decode_arithmetic(model, enc, tokenizer, inp, prompt=None, embed=None, finish_sent=False, device=None, temp=1.0, precision=16, topk=50000):
    ...
    with torch.no_grad():
        ...
        while i < len(inp):
            ...
            # Update history with new token
            next_token = torch.tensor([inp[i]], device=device, dtype=torch.long).unsqueeze(0)
            next_token_embed = model.gpt.transformer.wte(next_token)
            generated = torch.cat((generated, next_token_embed), dim=1)
            i += 1
